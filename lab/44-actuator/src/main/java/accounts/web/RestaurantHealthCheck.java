...
public class RestaurantHealthCheck implements HealthIndicator {
    ...
    public Health health() {
        var restaurantCount = repository.getRestaurantCount();
        if (restaurantCount > 0) {
            return Health.up().build();
        } else {
            //return Health.down().withDetail("restaurantCount", restaurantCount).build();
            return Health.status(new Status("NO_RESTAURANTS")).build();
        }
    }
}
